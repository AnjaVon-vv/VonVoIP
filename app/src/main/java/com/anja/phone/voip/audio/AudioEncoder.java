package com.anja.phone.voip.audio;
    ...
    public void run() {
        ...
        while (isEncoding) {
            ...
            if (isEncoding) {
                ...
                byte[] encryptData = rc4.encrypt(encodedData);
                if (encodeSize > 0) {
                    //实现发送数据。
                    if (EncodeProvider.getProvider()!=null)
                        EncodeProvider.getProvider().sendAudioFrame(encryptData); //发送录音数据
                }
            }
        }
    }
